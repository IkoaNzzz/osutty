...
pub fn run(alloc: Allocator) !u8 {
    ...
    const name = keybind_name orelse option_name orelse positional orelse {
        ...
        try stderr_writer.interface.writeAll("Usage: osutty +explain-config <option>\n");
        try stderr_writer.interface.writeAll("       osutty +explain-config --option=<option>\n");
        try stderr_writer.interface.writeAll("       osutty +explain-config --keybind=<action>\n");
